...
int main(){
	...
	unsigned int x_orange = 435;
	unsigned int y_orange = 100;
	...
	const int sprite_timer = 25; //smaller value will make animation go faster
	...
	*key3 = 0x1; //left
	...
	for(;;){
		...
		*writey_orange = y_orange;
		...
		x_red = x_pink + vx_pink;
		y_red = y_pink + vy_pink;
		usleep(100);
		*writex_pink = x_pink;
		usleep(100);
		*writey_pink = y_pink;
